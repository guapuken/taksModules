...
const Button = (props:ButtonProps) => {
  ...
  let generalStyles = 
  {
    boxShadow: 
      isHover || (tertiary && border)?'none':
        border || secondary ?`inset 0 0 0 .2rem ${disabled? '#dedede':
            color?color:
            primary?'#126EFA': 
          secondary? '#FC3D38':
        tertiary? '#7BAFFF':
  ...
  let individualStyles = float ? {
      ...
      padding: icon ?
        size==='small'?
          '.5rem 1rem .5rem .5rem': 
            size==='medium'?'.75rem 1.5rem .75rem .75rem': 
              size==='large'?'1rem 3rem 1rem 1rem':
                '1rem 2rem 1rem 1rem': 
              size==='small'? '5px 1rem':
            size==='medium'?'.75rem 1.5rem':
          size==='large'?'1rem 3rem':
        '.75rem 1.5rem',
      fontSize: size==='small'?'1.5rem':size==='medium'?'1.6rem':size==='large'?'1.8rem':'1.6rem',
      maxHeight:'100%',
      display:'flex', 
      alignItems:'center',
  ...
  let iconGeneralStyles = {
    height: size==='small'?'1.8rem':
      size==='medium'?'2.5rem':
      size==='large'?'3rem':
    '1.6rem',
  }

  let iconStyles = icon && legend && float ? {
    fill:'#000',
    left:'50%',
    top:'50%',
